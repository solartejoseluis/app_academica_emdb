...
> Versión: 104 — agrega la barra de pendientes y el detalle de requisitos en la vista del Estudiante (commit `ac76748`), cerrando por completo la Etapa 2.12.I (I1+I2) y con ella **toda la funcionalidad nueva** del roadmap "Gestión de requisitos de estudiantes" (2.12.0 a 2.12.I) — solo resta la Etapa 2.12.J de documentación de cierre. Nueva barra `#barra_requisitos_pendientes` (`reportes_view.php`), envuelta en `<?php if (!$es_coordinador): ?>`, ubicada antes de `#tabsProgramas`/`#contenidoProgramas` (el único punto fijo del layout independiente de la pestaña activa). `cargarResumenRequisitosEstudiante()` (`reportes_ctrl.js`) consume `resumen_requisitos_estudiante` (v103) y decide oculta/verde/roja según `total_global`/`entregados_global`, guardando además `por_matricula` en `porMatriculaRequisitos` para uso de otras funciones. Nuevo contenedor `.requisitos-programa-container` por pestaña en `cargarProgramas()` (entre el encabezado y `.sel-periodo`) y nueva `cargarDetalleRequisitosPestana()`: sin AJAX si la matrícula no tiene requisitos, si los tiene consulta el endpoint nuevo `mis_requisitos_matricula` (`reportes_mdl.php`) y puebla una tabla de solo lectura, con carga diferida por pestaña (mismo patrón `periodosLoaded`/`cargarPeriodos()`). **Bug de condición de carrera detectado y corregido:** `cargarProgramas()`/`cargarResumenRequisitosEstudiante()` corren en paralelo, pero la primera pestaña podía cargar su detalle antes de que `porMatriculaRequisitos` estuviera listo, dejando la sección silenciosamente vacía — corregido capturando el `jqXHR` en `resumenRequisitosPromise` y esperándolo con `$.when(...).always(...)` antes de decidir, sin volver secuenciales las llamadas ni romper el caso Coordinador (`$.when(null)` resuelve de inmediato). `mis_requisitos_matricula` se creó **separado** de `listar_requisitos_matricula` (`est_mdl.php`, Etapa 2.12.D) — ese endpoint no verifica pertenencia del `matr_id`, seguro solo porque sus consumidores actuales son Admin/Coordinador con alcance amplio; el nuevo agrega esa verificación (mismo patrón de `mis_periodos`/`detalle_periodo`). Verificado con Playwright + `curl` en 4 casos: tabla poblada para pestaña con requisitos, contenedor vacío para pestaña sin ellos, `"Sin autorización"` al pedir el `matr_id` de otro estudiante, y carga diferida confirmada por Network (la segunda pestaña nunca dispara AJAX si no tiene requisitos). Datos de prueba eliminados al terminar. Nota operativa: el commit original (`b30e084`) quedó con el mensaje mutilado por expansión de historial de bash (`!$es_coordinador` fuera de comillas simples) y se corrigió con `git commit --amend` al hash final `ac76748`.
