...
> Versión: 95 — agrega el backend Coordinador para el checklist de requisitos por matrícula (commit `350c614`), cerrando por completo el backend de la Fase 2.12 (etapas 2.12.0 a 2.12.D — todo lo que sigue es frontend): 2 `case` nuevos en `est_mdl.php`, guardados con `in_array($role_id, [1, 2], true)` (a diferencia del catálogo de la v93, aquí sí opera el Coordinador, no solo el Admin) — `listar_requisitos_matricula` (`SELECT` de `requisitos_estudiante` con `INNER JOIN` — deliberadamente no `LEFT` — hacia `requisitos_programa`, para que un requisito desactivado después en el catálogo siga siendo visible con su nombre y descripción en el historial de la matrícula que ya lo tenía asignado) y `actualizar_requisito_estudiante` (`reqe_id` + `reqe_estado`, validado estricto contra `['pendiente', 'entregado']`; marcar `'entregado'` fija `reqe_fecha = CURDATE()`, revertir a `'pendiente'` la deja en `NULL`). `listar_matriculados` gana 3 subqueries correlacionadas por `m.matr_id` — `requisitos_total`, `requisitos_entregados`, `requisitos_ultima_actualizacion` — mismo patrón que las ya existentes `total_matriculas_estudiante`/`soac_estado_activo`, sin quitar ninguna columna previa. Verificado con `curl` contra Docker vivo, autenticando como Coordinador: el `INNER JOIN` trajo nombre/descripción correctos; el ciclo entregado→pendiente dejó `reqe_fecha` en hoy y luego en `NULL` respectivamente, confirmado con SQL directo; un `reqe_estado` inválido fue rechazado sin tocar la fila; `listar_matriculados` mostró las 3 columnas nuevas en las 19 filas de la respuesta, con `0`/`0`/`null` en las matrículas sin ningún requisito y valores correctos en las que sí tenían. Datos de prueba eliminados al terminar. Etapa 2.12.D (de 10) del roadmap "Gestión de requisitos de estudiantes"; `est_view.php`/`est_ctrl.js` siguen sin tocarse — arrancan en la Etapa 2.12.E.
