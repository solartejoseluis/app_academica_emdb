...
| ~~UI de segunda matrícula / multi-programa~~ (RESUELTO 2026-08-30, commit `f5c21e5`) | El esquema ya permitía que un estudiante tuviera más de una fila en `matriculas` desde `3e551e5`, pero no existía ninguna UI para que el coordinador matriculara deliberadamente a un estudiante ya matriculado en un segundo programa. Implementado en `f5c21e5`: botón "➕ Matricular en otro programa" en `tablaMatriculados` (reutiliza `abrirMatricular()`/`#mdl_matricular`), contexto de matrículas previas en ese modal, sección "Programas matriculados" en el modal Datos Estudiante, validaciones que bloquean duplicar el mismo programa en `matricular`/`editar_matricula`, y fix del bug real de `total_modulos` que mezclaba módulos entre programas (ver CHANGELOG.md). **Quedó fuera de este alcance, deliberadamente:** ningún indicador visual tipo "2 programas" agregado a las filas de Matriculados/Aspirantes — cada matrícula sigue mostrándose como una fila independiente del listado (`listar_matriculados`/`listar_aspirantes` sin cambios de estructura, siguen sin `LIMIT`/agrupación); se evaluó y se descartó en esta ronda. Si en el futuro se quiere un indicador agregado por estudiante (no por matrícula), requiere decisión explícita de Jose Luis sobre el diseño del listado. |
