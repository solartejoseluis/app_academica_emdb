...
| `matr_estado_academico` (commit `3e551e5`, 2026-08-30) existe en el esquema pero ningún endpoint lo lee ni lo escribe | La columna se agregó con su default `'Activo'` como preparación de esquema para la funcionalidad de multi-programa — ningún `case` de `est_mdl.php` ni de ningún otro `_mdl.php` la incluye todavía en ningún `SELECT`/`INSERT`/`UPDATE`. Pendiente: diseñar dónde y cómo se edita (¿modal de matrícula? ¿acción separada tipo `toggle_estado_academico`?) antes de cablearlo. No implementar sin decisión explícita de Jose Luis. |
