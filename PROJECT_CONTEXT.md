...
| UI de segunda matrícula / multi-programa (commit `3e551e5`, 2026-08-30) | El esquema ya soporta que un estudiante tenga más de una fila en `matriculas` (cada una con su propio `prog_id`/`coho_id`/`matr_estado_academico`) — de hecho `case 'matricular'` en `est_mdl.php` nunca bloqueó esto (ver diagnóstico previo a este commit). Pero no existe ninguna UI para que el coordinador matricule deliberadamente a un estudiante ya matriculado en un segundo programa, ni para que los listados (`listar_matriculados`, `listar_aspirantes`) distingan/agrupen las filas de un mismo estudiante con más de una matrícula — ambos siguen con `INNER`/`LEFT JOIN` simple sin `LIMIT`, expuestos a fan-out de filas si esto llegara a ocurrir (mismo hallazgo ya documentado en el diagnóstico previo a `3e551e5`). Pendiente de decisión futura: diseño de la UI de segundo programa y manejo explícito de multi-matrícula en los listados. No implementar sin decisión explícita de Jose Luis. |
