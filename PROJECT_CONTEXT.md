...
> Versión: 85 — Agrega tabla solicitudes_actualizacion (Fase 1/5, commit `3ba9f99`): primera fase de un feature nuevo en 02_estudiantes — link público (sin sesión) para que un estudiante actualice su propia ficha (estudiantes + fichas_inscripcion), pendiente de que el coordinador apruebe o descarte los cambios desde un nuevo ítem del dropdown de Acciones de tablaMatriculados; precedido de un diagnóstico de solo lectura (sin commit) que mapeó el esquema de ambas tablas, el modal #mdl_estudiante, el patrón "público pero seguro" ya usado en 09_inscripcion_publica/, el case guardar_completo (candidato a reutilizar/adaptar en la aprobación), el patrón de sincronización usua_email de doc_mdl.php, la estructura de tablaMatriculados, y la convención de nombres del proyecto (confirmó que soac_/soac_ficha_ no chocan con ningún prefijo existente); nueva tabla solicitudes_actualizacion (database/emdb_academica.sql, sección "BLOQUE 6D") con 63 columnas — 10 de control/auditoría (soac_id PK, estu_id FK→estudiantes ON DELETE CASCADE, soac_token UNIQUE, soac_generado_en/soac_expira_en, soac_estado ENUM('generado','recibido','aprobado','descartado'), soac_recibido_en/soac_resuelto_en, soac_resuelto_por FK→usuarios ON DELETE SET NULL, soac_campos_modificados), 17 espejo de estudiantes (prefijo soac_estu_, todas NULLABLE aunque el original sea NOT NULL, excluye estu_tipodoc/estu_numerodoc y columnas de identidad/sistema) y 35 espejo de fichas_inscripcion (prefijo soac_ficha_, excluye finc_id/estu_id/finc_estado/fechacreacion); 3 decisiones de diseño documentadas — soac_ficha_prog_id sin FK hacia programas (valor propuesto sin validar hasta la aprobación, no en el momento de guardar la solicitud), soac_estado como borrado lógico en vez de DELETE físico (una solicitud descartada transiciona de estado, nunca se elimina, mantiene trazabilidad completa), tabla ubicada al final del .sql en su propia sección "BLOQUE 6D" (mismo patrón que ayudas/configuracion, sin renumerar el "Orden de creación" de CLAUDE.md); CREATE TABLE aislado aplicado contra el contenedor Docker vivo, sin re-ejecutar el .sql completo (habría dropeado las 18 tablas existentes); footer del .sql actualizado 18→19 tablas; verificado con DESCRIBE (63 columnas correctas), information_schema.KEY_COLUMN_USAGE (ambas FK registradas), SHOW INDEX (PRIMARY + uq_soac_token) y grep de soac_/solicitudes_actualizacion en todo app/ (0 coincidencias, sin colisión de nombres); **Fase 1 de 5 — el feature NO está cerrado**: faltan Fase 2 (backend de generación de token), Fase 3 (formulario público que consume el token), Fase 4 (frontend del dropdown/badge en tablaMatriculados + aprobar/descartar), y Fase 5 (documentación de cierre) + Matricular al siguiente semestre (02_estudiantes, commit `0aef564`): pieza final del roadmap de "semestre en la UI", CIERRA el roadmap completo (c985188 → 8884cb4 → 0aaa4e9 → 27a3915 → 0aef564); backend: validarSemestrePrograma() extraída como función reutilizable en est_mdl.php (antes duplicada literal en matricular/editar_matricula), ahora usada también por el nuevo case; listar_matriculados agrega matr_estado_academico y aprobado_periodo_actual (subconsulta que replica de forma consciente la lógica de promedio de detalle_periodo — duplicación deliberada de esa lógica de negocio, igual que el umbral 3.0 ya duplicado en ~15 lugares del proyecto, porque extraerla a una función compartida entre dos módulos distintos sería más disruptivo que el problema que resuelve; solo para decidir qué mostrar en el dropdown, nunca para autorizar); nuevo case 'avanzar_semestre' (solo coordinador/administrador) con validaciones en cadena — matr_estado='matriculado', matr_estado_academico='Activo', límite de semestre vía validarSemestrePrograma(matr_semestre+1), período destino distinto del actual, aprobación RECALCULADA con query directa en el momento de escribir (no confía en aprobado_periodo_actual ni en ningún valor por POST), sin fila duplicada en el destino — y una transacción PDO que pasa la fila anterior a matr_estado='cursado' (valor reservado desde c985188) e inserta la nueva con matr_semestre+1 y el mismo coho_id heredado; frontend: crearColumnasMatriculados(esPeriodoActual) gana un parámetro booleano (decisión documentada: no viola la advertencia de CLAUDE.md contra funciones compartidas con parámetros condicionales porque solo 1 de 13 columnas difiere, no la estructura completa), nuevo ítem de dropdown "⏩ Matricular al sgte. sem." visible solo en Per. Actual con 4 motivos de deshabilitado en cascada (matr_estado, matr_estado_academico, último semestre, no aprobado — el primero que aplique se muestra en el tooltip), nuevo modal #mdl_avanzar_semestre con contexto de solo lectura y selector de período destino poblado clonando las opciones ya excluidas del activo de #slct_filtro_matr_peri_id_anteriores en vez de exponer periodoActivoId fuera de su scope (esa variable es local a $(document).ready, la función que abre el modal es global); backend probado con curl 10/10 casos (avance exitoso, reintento sobre fila cursado, notas incompletas, aplazamiento, último semestre, promedio <3.0, duplicado en destino, mismo período como destino, sin sesión — todos los datos de prueba temporales y los 2 avances reales de prueba revertidos sin dejar rastro); verificado en navegador por Jose Luis 6/6 puntos; con esta pieza, el roadmap completo de "semestre en la UI" queda cerrado — no queda ningún pendiente de esa serie (distinto del pendiente ya documentado, y todavía abierto, de un mecanismo de EDICIÓN de matr_estado_academico en sí, sin relación con esta acción de avance) + Agrega semestre del estudiante a reporte y boletín PDF (06_reportes, commit `27a3915`): completa la tercera pieza del roadmap de "semestre en la UI" (después de c985188, 8884cb4 y 0aaa4e9, todos 2026-09-02); mis_programas y detalle_periodo (reportes_mdl.php) agregan matr_semestre y prog_duracion_semestres al SELECT (mismo JOIN existente hacia programas, sin JOIN nuevo); reportes_ctrl.js agrega formatoSemestre() (formato "N/M", fallback '—') y muestra "Semestre" en el bloque de contexto de cargarProgramas() junto a Programa/Cohorte/Períodos realizados/Estado actual (reparto de columnas Bootstrap ajustado de 4/3/3/2 a 4/2/2/2/2); pdf_boletin.php extiende el mismo SELECT y muestra "Semestre: N/M" en el par de celdas vacías ya reservadas en la fila de "Período" de la tabla de contexto, sin agregar fila nueva (respeta el antipatrón de Dompdf con colspan mezclado ya documentado en CLAUDE.md); 05_calificaciones (vista del docente) queda fuera de alcance por decisión explícita — grse_semestre del grupo, ya visible ahí como "Sem.N", se conserva sin cambios; no se agrega matr_semestre a esa vista porque en la lógica de negocio ambos deberían coincidir, y mostrar los dos por separado generaría ruido en vez de claridad; verificado en navegador por Jose Luis 3/3 pruebas (dato visible en el reporte, estable al cambiar de período, boletín PDF con layout correcto); con esta pieza, el único punto pendiente del roadmap de semestre es ahora la acción "avanzar de semestre" — el resto de piezas de visualización (columna en listado, pestañas Per. Actual/Anteriores, reportes, boletín) quedan completas + Divide Matriculados en pestañas Per. Actual / Per. Anteriores (02_estudiantes, commit `0aaa4e9`): segunda pieza visible del roadmap de "semestre en la UI" (después de c985188 y 8884cb4, ambos 2026-09-02); precedido de un diagnóstico de solo lectura (sin commit) que confirmó que tablaMatriculados tenía un único select de período sobre una sola tabla, sin distinguir activo de históricos, que listar_periodos no exponía peri_activo, y que listar_matriculados no tenía m.peri_id en su SELECT pese a que verModulosEstudiante() lo necesitaba; agrega peri_activo a listar_periodos y m.peri_id a listar_matriculados (est_mdl.php); pestaña única "Matriculados" dividida en dos pestañas de nivel superior — "Matriculados (Per. Actual)" con el período fijo al activo vía input readonly, sin selector, y "Matriculados (Per. Anteriores)" con un select que lista todos los períodos excepto el activo, sin opción "Todos", preseleccionando el período anterior más reciente; cada pestaña con su propia tabla (tbl_matriculados_actual/tbl_matriculados_anteriores) y su propio bloque de filtros Programa/Grupo/Módulo; columns extraído a crearColumnasMatriculados(), función global que retorna un array nuevo en cada llamada — primer caso del proyecto de dos instancias de DataTable con la misma estructura de columnas, sin precedente previo en 04_grupos/03_docentes (cada tabla ahí tiene columnas distintas); verModulosEstudiante() recibe peri_id como 4º parámetro explícito (row.peri_id) en vez de leer el selector único que dejó de existir; las 5 acciones que antes recargaban una sola tabla (guardar estudiante, subir foto, matricular, cerrar matrícula, editar matrícula) ahora recargan ambas, con guardas if (tabla) por si alguna no llegó a inicializarse; 4 decisiones por ambigüedad documentadas: m.peri_id faltante en listar_matriculados pese a que la especificación lo daba por existente; sin opción "Todos" en el select de Anteriores (habría enviado peri_id vacío, colando el período activo en la pestaña de históricos) — se preselecciona el anterior más reciente en su lugar; edge case de cero períodos anteriores resuelto con value="0" (no coincide con ningún peri_id real, la tabla responde 0 filas en vez de cae en "sin filtro"); doble carga inicial por la carrera entre cargarTablas()/cargarPeriodos() — corregida con un ajax.reload() forzado de ambas tablas apenas periodoActivoId se resuelve, sin impacto visible más allá de un round-trip HTTP extra en la carga inicial; verificado con curl autenticado contra el contenedor Docker vivo (peri_activo presente, m.peri_id presente en las 17 filas, filtrado correcto por período activo/anterior/edge-case) y confirmado por Jose Luis en navegador 6/6 pruebas; pendiente como próximos pasos de este mismo feature: semestre en reportes/boletín/vista del docente, y la acción "avanzar de semestre" en sí (la única que escribiría el ENUM 'cursado' reservado desde c985188) + Agrega columna "Semestre" a tablaMatriculados (02_estudiantes, commit `8884cb4`): primera pieza visible en la UI del trabajo de base sentado en c985188 — matriculas.matr_semestre existía en el esquema y ya se capturaba en los modales de matrícula, pero no se mostraba en ningún listado hasta ahora; precedido de un diagnóstico de solo lectura (sin commit) que confirmó que tablaMatriculados no usa columnDefs (depende 100% del orden literal del array columns frente al <thead>, insertar en medio requiere sincronizar ambos) y que listar_matriculados no tenía matr_semestre ni prog_duracion_semestres en su SELECT pese a que el JOIN hacia programas ya existía (se usa para prog_sigla); case listar_matriculados (est_mdl.php) agrega ambos campos al SELECT existente, mismo JOIN, sin JOIN nuevo — único consumidor de ese case en todo el proyecto, adición aditiva sin riesgo; nuevo <th>Semestre</th> entre "Programa" y "Período" en est_view.php, nueva columna en la misma posición del array columns en est_ctrl.js (width 60px, tomado del espacio que ya absorbían las columnas sin width fijo); render() muestra "N/M" (matr_semestre/prog_duracion_semestres) con fallback '—' si falta cualquiera de los dos, resaltado suave #cfe2ff (mismo estilo inline que la columna Edad) cuando matr_semestre === prog_duracion_semestres (último semestre); decisión técnica: comparación con == en vez de === en JS, porque PDO devuelve los campos numéricos como strings vía JSON — una comparación estricta habría fallado silenciosamente sin aplicar nunca el resaltado; verificado con curl contra el contenedor Docker vivo (SELECT correcto en las 17 filas actuales) y con 2 filas reales editadas temporalmente en MySQL para forzar ambas ramas del render (2/5 sin resaltado, 5/5 con resaltado), revertidas de inmediato sin dejar datos de prueba; verificación visual en navegador NO realizada en esta sesión por falta de herramienta de navegador disponible, pendiente de confirmación por Jose Luis; pendiente como próximos pasos de este mismo feature: pestañas Per. Actual/Per. Anteriores, el dato de semestre en reportes/boletín/vista del docente, y la acción "avanzar de semestre" en sí (la única que escribiría el ENUM 'cursado' reservado desde c985188) + Agrega matr_semestre a matriculas con validación y corrige badge de programas (02_estudiantes, commit `c985188`): nueva columna matriculas.matr_semestre (TINYINT UNSIGNED NOT NULL DEFAULT 1, 1-N) — precede a la futura acción "avanzar de semestre" (matricular a un estudiante ya matriculado a su siguiente semestre dentro del mismo programa), motivada por un diagnóstico previo de solo lectura (sin commit) que concluyó que el esquema ya soporta múltiples filas por estu_id+prog_id (uq_matr_estu_peri_prog solo exige peri_id distinto) y que el código ya tiene precedente para resolver "la matrícula vigente" con varias filas por estudiante (ORDER BY matr_id DESC LIMIT 1, ya usado en pdf_hoja_matricula.php/obtener_completo) — la alternativa de nueva fila por semestre rompe menos código existente que actualizar la fila existente; validada en backend (case matricular y case editar_matricula de est_mdl.php) contra programas.prog_duracion_semestres del prog_id de la matrícula, rechaza con "Semestre inválido para este programa" fuera de rango, default explícito a 1 si no llega por POST (no confía en el DEFAULT de columna sin pasar por la misma validación); ENUM matr_estado ampliado con 'cursado' — valor reservado para esa futura acción de avance, confirmado por grep que ningún endpoint lo escribe todavía; select #slct_matr_semestre (Completar Matrícula) y #slct_editar_matr_semestre (Editar Matrícula) poblados de 1 a la duración del programa vía nueva función global poblarSelectSemestre(), leyendo data-duracion agregado a cada <option> de programa por cargarProgramas() (sin llamada AJAX adicional), misma razón de scope global que cargarCohortesPorPrograma (invocada tanto desde listeners de change dentro de ready como desde abrirMatricular()/abrirEditarMatricula() global); fix de bug preexistente: el badge "🎓 N programas" (1529832) usaba COUNT(*) de filas de matriculas como proxy de "número de programas" — válido mientras cada estu_id+prog_id tenía máximo una fila, pero matr_semestre permite ahora varias del mismo programa (una por semestre cursado), lo que habría inflado el conteo; corregido a COUNT(DISTINCT prog_id), mismo criterio que programas_matriculados_activos; obtener_completo (modal Datos Estudiante) recibe matr_semestre en el SELECT sin renderizarlo en UI — ese modal no es el editable de matrícula, mostrar el dato ahí se acerca a la columna de listado fuera de alcance; verificado con curl autenticado + SQL directo contra el contenedor Docker vivo (default a 1 sin enviar el campo; rechazo de semestre 5 en programa de duración 4 con rollback confirmado + edición válida posterior a semestre 3; badge devolviendo 1 con 2 matrículas del mismo programa en períodos distintos, antes habría sido 2), datos de prueba limpiados al finalizar; php -l detectó y permitió corregir un error real antes de tocar la BD (comentario SQL con comillas dobles cerraba prematuramente un string PHP); próximos pasos de este mismo feature, NO implementados aún: columna "Semestre" en el listado de tablaMatriculados, pestañas Per. Actual/Per. Anteriores, reportes con "Semestre: N/M", y la propia acción "avanzar de semestre" (la que efectivamente escribiría 'cursado') + Agrupa Acciones de tablaDocentes en dropdown con ícono de tuerca (03_docentes, commit `e1c232d`): aplica el mismo patrón de dropdown ya usado en 02_estudiantes (tablaMatriculados `4fb2257`, tablaAspirantes `9346a76`) a la columna "Acciones" de tablaDocentes; a diferencia de esos 2 casos, esta columna nunca acumuló 3+ botones simultáneos (siempre 2 visibles a la vez: "Editar" fijo + uno solo de Eliminar/Desactivar/Activar, mutuamente excluyentes según doce_activo/total_grupos_historico, nunca los tres a la vez) — no cruzaba el umbral de "3+ acciones" documentado en CLAUDE.md que originalmente motivó el patrón; se aplicó igual por decisión explícita de Jose Luis de uniformidad visual en toda la aplicación, no por saturación; a diferencia de Matriculados/Aspirantes (que sumaron un ítem nuevo antes solo accesible en otro modal), aquí no se agrega ninguna funcionalidad nueva — el dropdown solo reagrupa visualmente lo que ya existía; la lógica condicional if/else if/else sobre doce_activo/total_grupos_historico se preservó sin ningún cambio, solo cambió el HTML generado en cada rama (de botón en línea a dropdown-item), igual que el cálculo de nombreCompleto y los mismos onclick (abrirEditar, confirmarEliminarDocente, toggleEstadoDocente); CDN de Bootstrap Icons agregado a doc_view.php por primera vez en ese archivo; columna Acciones reducida de 190px a 70px; sin cambios de backend (doc_mdl.php intacto); verificado en navegador 5/5 pruebas, incluidos los 3 casos de la lógica condicional (Eliminar/Desactivar/Activar) + Agrupa Acción de Aspirantes en dropdown con ícono de tuerca (02_estudiantes, commit `9346a76`): aplica a tablaAspirantes el mismo patrón de dropdown ya implementado en tablaMatriculados (commits `53d5ec5`/`4fb2257`) — reemplaza los 2 botones en línea de la columna Acción (Matricular, 📝 Datos Estudiante) por un único botón de ícono bi-gear-fill que despliega un dropdown de Bootstrap (dropdown-menu-end, data-bs-boundary="viewport") con 3 acciones: Matricular y 📝 Datos Estudiante sin cambio de comportamiento (mismo onclick, esAspirante=true; punto de color de estado_ficha conservado, recalculado localmente en este render(), sin compartirse con tablaMatriculados) y un ítem NUEVO 🖨️ Ficha de Inscripción (antes solo accesible dentro del modal Datos Estudiante vía #btn_ficha_inscripcion_pdf, condicionado a estado_ficha === 'completa'; el nuevo ítem del dropdown es un acceso directo sin esa validación en frontend, misma decisión explícita — Opción A — ya aplicada a Hoja de Matrícula en Matriculados); columna Acción reducida de 300px a 70px; a diferencia del trabajo de Matriculados, esta tarea fue un solo commit de frontend puro, sin fase de backend, porque estado_ficha ya viajaba en cada fila de listar_aspirantes; tablaMatriculados no fue tocada; verificado en navegador 7/7 pruebas, incluida la descarga de Ficha de Inscripción para un aspirante con ficha incompleta (comportamiento esperado, sin bloqueo) + Deshabilita "Matricular en otro programa" sin programas activos disponibles + agrupa Acción de Matriculados en dropdown (02_estudiantes, commits `53d5ec5` y `4fb2257`): tarea de 2 fases sobre la columna "Acción" de tablaMatriculados; backend (53d5ec5) agrega 2 columnas aditivas a listar_matriculados sin tocar JOINs/WHERE/forma de la respuesta (sigue una fila por matrícula) — programas_activos_totales (COUNT(*) de programas con prog_activo=1, mismo valor repetido en cada fila) y programas_matriculados_activos (COUNT(DISTINCT prog_id) de matrículas del estudiante con matr_estado='matriculado' Y prog_activo=1, replicando exactamente la misma condición de matr_estado ya usada en los cases matricular/editar_matricula, para no introducir una regla de negocio distinta a la ya validada ahí); frontend (4fb2257) reemplaza los 3 botones en línea de Acción (📝 Datos Estudiante, ✏️ Matrícula, ➕ Matricular en otro programa) por un único botón de ícono (bi-gear-fill, Bootstrap Icons 1.11.3 vía CDN jsdelivr, nuevo <link> en est_view.php) que despliega un dropdown de Bootstrap (dropdown-menu-end, data-bs-boundary="viewport") con 4 acciones — los 2 primeros ítems sin cambio de comportamiento (punto de color de estado_ficha conservado, recalculado localmente, sin compartirse con tablaAspirantes que no fue tocada), un ítem NUEVO "🖨️ Hoja de Matrícula" (antes solo accesible dentro del modal Datos Estudiante vía #btn_hoja_matricula_pdf condicionado a matr_estado==='matriculado'; el nuevo ítem del dropdown es un acceso directo sin esa validación en frontend, decisión explícita) y "➕ Matricular en otro programa" ahora deshabilitado con tooltip cuando programas_matriculados_activos >= programas_activos_totales (envuelto en <li tabindex="0" title="..."> por ser un <button disabled>, patrón estándar de Bootstrap para tooltips sobre ítems deshabilitados); columna Acción reducida de 360px a 70px; verificado en navegador (dropdown sin recorte en última fila, ítems sin cambio de comportamiento, PDF descarga correctamente, botón deshabilitado con tooltip para estudiante matriculado en ASO+MD, tablaAspirantes sin cambios) + Indicador visual "N programas" en Matriculados (02_estudiantes, commit `1529832`): cierra la última pieza pendiente del ciclo de matrícula a segundo programa — f5c21e5 (2026-08-30) evaluó y descartó explícitamente un indicador visual tipo "2 programas" en su sección "Decisiones"; retomado a pedido explícito de Jose Luis; 2 columnas nuevas en listar_matriculados: total_matriculas_estudiante (COUNT correlacionado SOLO por estu_id, deliberadamente sin filtrar por prog_id/peri_id de la fila — a propósito lo opuesto de total_modulos, que f5c21e5 corrigió para sí filtrar por la matrícula de la fila actual; documentado con comentario SQL inline para no confundirlo con ese mismo antipatrón) y detalle_matriculas_estudiante (GROUP_CONCAT "SIGLA — Estado" de todas las matrículas del estudiante); badge "🎓 N programas" junto al apellido en tablaMatriculados, visible solo si total_matriculas_estudiante > 1, con el detalle en el title; tablaAspirantes sin ningún cambio, confirmado por diseño (un aspirante no tiene aún una segunda matrícula que mostrar); sin endpoint nuevo — se descartó tanto reutilizar matriculas_estudiante vía AJAX por fila como crear uno nuevo, a favor de traer el detalle agregado en la misma query (evita N llamadas HTTP extra); verificado con curl (estu_id=12, MD+ASO, total_matriculas_estudiante=2 en ambas filas con detalle correcto; resto de estudiantes en 1; listar_aspirantes confirmado sin cambios) + Matrícula a segundo programa + fix de total_modulos cruzado entre programas (02_estudiantes, commit `f5c21e5`): cierra el ciclo abierto por 3e551e5, agrupando (nunca se comiteó el paso intermedio) tanto la UI de "matricular en otro programa" como 2 fixes reales; funcionalidad nueva: botón "➕ Matricular en otro programa" en tablaMatriculados (reutiliza abrirMatricular()/#mdl_matricular existente), contexto de "Programas ya matriculados" en ese modal, sección "Programas matriculados" (con fecha) en el modal Datos Estudiante, nuevo case matriculas_estudiante, obtener_completo ampliado con el array matriculas; validaciones nuevas: matricular y editar_matricula bloquean crear/editar una matrícula que duplique un programa ya matriculado del mismo estudiante en otro período, mismo mensaje explícito en ambos; fix eliminar_aspirante: el chequeo usaba fetch() de una sola fila sin ORDER BY (no determinista con 2+ matrículas) — ahora fetchAll()+foreach revisa TODAS las filas antes de decidir, y borra todas las 'aspirante' antes del DELETE de estudiantes; fix total_modulos: listar_matriculados y su modal de detalle (listar_modulos_estudiante) contaban/listaban módulos de TODOS los programas del estudiante en cada fila en vez de solo el de esa matrícula — corregido cruzando gs3.prog_id = m.prog_id (con alias matr_prog_id, distinto de fi.prog_id ya existente, para evitar colisión silenciosa en FETCH_ASSOC) y agregando prog_id opcional a listar_modulos_estudiante; caso real verificado (estu_id=12, MD+ASO): antes ambas filas mostraban total_modulos=2, después ASO=0 y MD=2; no se agregó indicador visual "2 programas" en los listados — evaluado y descartado en esta ronda + Combina Reporte por Grupo + Reporte por Estudiante en pestañas (06_reportes, commit `8d479a1`): cierra el rediseño de 06_reportes — el plan original de la Fase 3 reemplazaba por completo "Reporte por Grupo" (consulta por grupo módulo, todos los estudiantes) con la nueva "Reporte por Estudiante" (informe por programa→período→módulo, con buscador para que el coordinador consulte a cualquier estudiante); a pedido explícito de Jose Luis ese reemplazo se corrigió en el mismo commit final para que ambas interfaces convivan como 2 pestañas de nivel superior (solo roles 1/2) — "Reporte por Grupo" recuperado tal cual del commit ef429bd (cargarGrupos/construirContextoTexto/escaparXml/mostrarInfoReporteGrupo/construirFilaReporte + DataTable con export Excel/PDF), "Reporte por Estudiante" sin cambios respecto a la Fase 3; estudiante (role 4) sigue sin pestañas de nivel superior, directo a su informe propio; 0 funciones JS duplicadas (verificado por grep), DataTables/Buttons/JSZip gateados por $es_coordinador; verificado con curl (HTTP 200 sin warnings, balance de <div> 38/38, 0 vs 1 ocurrencias de los bloques nuevos según el rol) — interacción entre pestañas dejada para prueba en navegador + Reescritura de pdf_boletin.php a boletín por período (06_reportes, commit `ef429bd`, Fase 2 de 3): reemplaza el boletín de un solo módulo (fetch(), WHERE grmo_id=?) por un boletín de TODOS los módulos de un período+programa de un estudiante (parámetros matr_id+peri_id, mismo criterio de resolución de estudiante objetivo que reportes_mdl.php, 403 genérico si el matr_id no pertenece al estudiante resuelto); encabezado institucional leído de configuracion.institucion_nombre (único de los 4 PDF del proyecto que ya no lo hardcodea); Estado del Período replicado del mismo cálculo de detalle_periodo; tabla .ficha de 11 columnas por módulo (incluye Habilitación, agregada en un ajuste posterior del mismo commit tras detectarse la omisión); bug encontrado y corregido en el camino: institucion_nombre había quedado doble-codificado en UTF-8 por un ALTER/UPDATE ejecutado sin --default-character-set=utf8mb4 (corregido y verificado con HEX()) — diagnóstico aparte confirmó que director_nombre/secretario_nombre NO tenían ese problema, solo un artefacto de terminal; verificado generando 2 PDFs reales (módulo Aprobado y módulo En Curso) con pdfinfo/pdftotext + Backend de reportes por período/programa (06_reportes, commit `b3a0296`, Fase 1 de 3): agrega configuracion.institucion_nombre (mismo patrón que director_nombre/secretario_nombre); 4 cases nuevos en reportes_mdl.php — mis_programas (matrículas del estudiante con prog_nombre/coho_codigo/matr_estado_academico), mis_periodos (períodos distintos con módulos asignados por programa, histórico completo sin filtrar grmo_activo), detalle_periodo (módulos del período + Estado del Período agregado: En Curso si falta alguna definitiva, si no promedio simple decide Aprobado/Reprobado), buscar_estudiante (LIKE sobre nombres/apellidos/documento, mínimo 3 caracteres, máx 15 resultados, solo roles 1/2); helper resolverEstuIdObjetivo() (role 4 siempre su propio estu_id de sesión, roles 1/2 reciben estu_id por POST) reutilizado después por pdf_boletin.php; validación de pertenencia de matr_id al estudiante objetivo en cada case; probado con curl contra el contenedor vivo incluyendo casos borde (estu_id faltante, matr_id ajeno, estudiante sin matrículas, módulo sin definitiva) + Migra coho_id de estudiantes a matriculas + agrega matr_estado_academico (commit `3e551e5`): prepara el modelo de datos para que un estudiante pueda cursar más de un programa a la vez — cada matrícula/programa con su propia cohorte y su propia condición académica del semestre, en vez de una única cohorte compartida a nivel de estudiante; matriculas gana coho_id (SMALLINT UNSIGNED, FK → cohortes.coho_id, ON DELETE SET NULL) y matr_estado_academico (ENUM('Activo','Aplazamiento','Retirado') NOT NULL DEFAULT 'Activo', independiente de matr_estado — que sigue siendo el ciclo de vida del trámite, no la condición académica del semestre); estudiantes.coho_id se conserva como respaldo histórico sin ninguna escritura nueva (confirmado por grep); ALTER TABLE + backfill ya aplicados contra el contenedor Docker vivo, verificado 100% consistente (16/16 filas); listar_matriculados/matricular/obtener_matricula/editar_matricula (est_mdl.php) migrados a leer/escribir coho_id desde matriculas; pdf_hoja_matricula.php y el KPI/guard de eliminación de cohortes en grupos_mdl.php migrados igual; hallazgo colateral confirmado por grep: database/emdb_academica.sql nunca sembró datos transaccionales (estudiantes/matriculas/cohortes/gruposemestres/etc.), solo esquema + catálogos fijos — no había ningún INSERT que sincronizar, documentado como nota operativa en CLAUDE.md; UI de segunda matrícula/multi-programa y lectura/escritura de matr_estado_academico quedan pendientes para una fase futura + Filtro grmo_activo=1 en total_modulos de listar_grupos (04_grupos, commit `b8a583a`): corrige la inconsistencia documentada en CLAUDE.md tras el commit a39cb24 — total_modulos contaba TODOS los gruposmodulos del grupo semestre (activos e inactivos) sin que el nombre del campo lo señalizara, a diferencia de total_grupos_historico (doc_mdl.php), que sí distingue por nombre cuando la intención es contar todo; se agrega AND gm.grmo_activo = 1 a esa subconsulta, alineándola con total_estudiantes (mismo case, agregada en a39cb24) y con todos los demás conteos de gruposmodulos del proyecto que ya filtraban por activo (listar_docentes en grupos_mdl.php, doc_mdl.php, coordinador_mdl.php, calificaciones_mdl.php, reportes_mdl.php); diagnóstico previo (grep en todo el proyecto) confirmó que era el único caso de este patrón sin filtro no señalizado por naming; verificado contra datos reales: 0 gruposmodulos con grmo_activo=0 en la base local al momento del cambio, sin impacto visible en este entorno, pendiente de verificación en producción; cambio de una sola línea, sin tocar ningún otro archivo + Columna "# Estud" en Grupos Semestre (04_grupos, commit `a39cb24`): listar_grupos gana una nueva subconsulta escalar total_estudiantes (COUNT(DISTINCT ge.estu_id) vía grmoestudiantes→gruposmodulos, filtrado por grmo_activo = 1) — necesario porque un mismo estudiante puede estar matriculado en más de un módulo del mismo grupo semestre, evitando inflar el conteo; nuevo case listar_estudiantes_grupo recibe grse_id y retorna estudiantes distintos (misma condición grmo_activo = 1) con estu_apellidos/estu_nombres ordenados alfabéticamente; nueva columna "# Estud" en tbl_grupos (grupos_view.php) después de "Módulos", botón que abre el nuevo modal #mdl_estudiantes_grupo (título = grse_codigo, tabla de 1 columna "Apellidos, Nombres", mensaje "Sin estudiantes asignados" si viene vacío); nueva función global verEstudiantesGrupo() en grupos_ctrl.js en scope global (fuera de $(document).ready), replicando el patrón ya usado en verModulosEstudiante() de 02_estudiantes; decisión explícita: se filtró grmo_activo = 1 (consistente con el patrón ya usado en est_mdl.php para contar módulos de un estudiante) en vez de replicar la omisión de ese filtro que tiene el total_modulos existente en el mismo case listar_grupos — deja documentada esa inconsistencia como observación, sin corregirla en este commit (fuera de alcance); verificado en navegador: columna visible, modal con listado correcto sin duplicados, caso vacío con mensaje correcto, sin errores de consola + Corrección de truncamiento SQL en códigos de grupo/cohorte (04_grupos, commit `0e0a508`): grse_codigo y coho_codigo ampliados de VARCHAR(20) a VARCHAR(25) para alinear el esquema con la convención ya documentada en CLAUDE.md; bug determinístico afectaba a cualquier grupo semestre creado para programas de sigla larga (ASO2016, AMD2016, 7 caracteres) porque la fórmula de autogeneración consumía 13-14 caracteres fijos, dejando solo 6 de margen bajo la columna original; validación defensiva mb_strlen() > 25 agregada en guardar_grupo y guardar_cohorte antes de llegar a MySQL; reproducido y verificado vía curl + navegador + Cableado completo de doce_cedula en "Nuevo/Editar Docente" (03_docentes, commit `7ba02bc`): columna `docentes.doce_cedula` (existía sin uso desde el inicio del proyecto) ampliada de VARCHAR(15) a VARCHAR(20) y cableada en las 3 capas — nuevo campo opcional en el modal, sin selector de tipo de documento (a diferencia de estu_numerodoc), validación de unicidad server-side condicionada a valor no vacío (mismo criterio que matr_numero), guardado como NULL si llega vacío; case 'listar' sin cambios por alcance; pendiente aplicar el ALTER TABLE en producción; verificado 6/6 pruebas en navegador + Edición de correo electrónico en "Editar Docente" (03_docentes, commit `b6cc503`): campo `#npt_usua_email` deja de estar bloqueado en modo edición (antes forzado a readonly en `abrirEditar()`), label actualizado a "Correo electrónico (usuario de acceso)" para hacer explícito que es también el identificador de login; validación de unicidad server-side agregada en la rama de edición de `doc_mdl.php` (mismo patrón que `doce_sigla`, excluye el propio `usua_id`); `usua_email` agregado a ambos UPDATE de `usuarios` en esa rama (con y sin cambio de contraseña simultáneo); verificado 6/6 pruebas en navegador + Mejora de usabilidad en la Ficha de Inscripción, AC-FO-02 (commit `3672a0e`): cambio puramente de presentación (sin tocar modelo ni controladores JS, ningún `id`/`name` de input modificado) en la Sección "1. Datos del Aspirante/Estudiante" de `insc_view.php` y `est_view.php` (`#mdl_estudiante`) — campos de residencia (Dirección/Barrio/Ciudad) reordenados y renombrados con sufijo "(Residencia)", "Ciudad de nacimiento" reubicada junto a "Fecha de nacimiento", EPS/Ocupación/Estado civil consolidados en una sola fila de 3 columnas con Discapacidad aislada en su propia fila; `insc_view.php` alineado a la misma disposición que ya tenía `est_view.php` (traslado de "Expedido en" a la fila del documento, unificación de label "Tipo/Documento" → "Tipo de documento"); Sección "3. Información sobre Familiares" (Padre/Madre/Acudiente, ambos archivos): Dirección/Barrio/Ciudad reordenados con el mismo sufijo "(Residencia)", Teléfono renombrado a "Teléfono (Personal)"; motivación: hacer explícito para quien diligencia el formulario a qué corresponde cada dato (ej. distinguir "Teléfono (Personal)" de un eventual teléfono de otro tipo, o "Dirección (Residencia)" de otra dirección que pudiera tener un familiar); encabezados "1. Datos del Aspirante" (insc_view.php) y "1. Datos del Estudiante" (est_view.php) mantenidos intencionalmente distintos, decisión explícita del autor; mismo commit confirma la eliminación, ya decidida previamente y sin relación con este cambio, de `actualizar_ayudas.sql` + Registro y visualización de "último acceso" del usuario al sistema (commit `ac96f07`): nueva columna `usuarios.usua_ultimo_acceso` (`DATETIME NULL DEFAULT NULL`) en el esquema; nuevo helper compartido `formatearUltimoAcceso($ultimoAcceso, $fechaCreacion)` en `00_files/helpers.php` (primer archivo de funciones comunes del proyecto), formato `d/m/Y H:i` con fallback a `fechacreacion` si `usua_ultimo_acceso` es `NULL`, `'—'` si ambos lo son; `login_mdl.php` guarda en sesión el valor ANTERIOR de `usua_ultimo_acceso` (`usua_ultimo_acceso_anterior` + `usua_fechacreacion`) antes de ejecutar `UPDATE usuarios SET usua_ultimo_acceso = NOW()`, en el único flujo de autenticación de los 4 roles, antes de la lógica condicional de `doce_id`/`estu_id`; bloque "Nombre — Email — Último acceso" agregado en `navbar.php` (roles 1/2), `calificaciones_view.php` (nav real de Docente — no `doc_view.php`, que resultó inalcanzable para ese rol y se revirtió en el mismo ciclo) y `reportes_view.php` (Estudiante); columna "Último acceso" nueva en las tablas de Usuarios (`08_admin`, formateada server-side en `admin_mdl.php`), Matriculados (`02_estudiantes`, requirió `LEFT JOIN usuarios` nuevo en `listar_matriculados` por `usua_id` opcional) y Docentes (`03_docentes`, reutiliza el `INNER JOIN usuarios` ya existente); hallazgo colateral documentado como deuda técnica, no corregido: `est_view.php` y `grupos_view.php` tienen el mismo `<nav>` fallback inalcanzable para Docente que tenía `doc_view.php`, ver "Análisis pendiente" + Habilitación de cambio de contraseña de docente en "Editar Docente" (03_docentes, commit `35c312a`): campo de contraseña de `#mdl_docente` deja de ocultarse en modo edición, replicando el patrón ya usado en `08_admin` (`admin_mdl.php` case `editar` — vacío no cambia el hash, `password_hash()` bcrypt si viene texto); label cambiado a "Contraseña", placeholder dinámico ("Contraseña inicial" en creación, "Dejar vacío para no cambiar" en edición); visibilidad condicionada a que el docente tenga `usua_id` (`abrirEditar()` en `doc_ctrl.js`); `case 'obtener'` de `doc_mdl.php` expone `usua_id` en el SELECT para esa decisión; `case 'guardar'` (rama edición) combina `usua_activo` + `usua_passwordhash` en un solo UPDATE cuando llega contraseña no vacía; sin validación de longitud mínima (mismo criterio que 08_admin); deuda identificada y no resuelta en este commit: `case 'obtener'` sigue con INNER JOIN docentes↔usuarios, por lo que un docente con `usua_id NULL` causaría "Docente no encontrado" en vez de abrir el modal sin el campo — hoy 0 docentes en esa condición, documentado en "Análisis pendiente" + Unificación del formulario público de inscripción en un solo paso (commit `0e098bb`): el flujo de 2 pasos con guardado parcial (insc_view.php/Paso 1 + fam_view.php/Paso 2, puenteados por finc_codigotemporal para "retomar después") se reemplaza por un solo formulario de una sola pantalla con un solo guardado transaccional; insc_view.php absorbe el HTML de fam_view.php en 5 secciones numeradas con el mismo diseño visual .seccion-titulo/.campo-lleno del modal interno de 02_estudiantes (commit f088466); insc_ctrl.js absorbe toda la lógica de fam_ctrl.js (cascada Padre/Madre/Acudiente, catálogo de Programas) en un solo submit AJAX; insc_mdl.php fusiona crear_aspirante + completar_ficha en un solo case con una sola transacción PDO (INSERT estudiantes + INSERT completo de fichas_inscripcion, ya no hay UPDATE posterior) — mismo principio ya documentado para guardar_completo ("dos transacciones individualmente correctas no garantizan atomicidad entre sí"); fam_view.php/fam_mdl.php/fam_ctrl.js eliminados por completo; columna finc_codigotemporal y su UNIQUE KEY eliminados del esquema (ALTER TABLE ya ejecutado manualmente en Docker el 2026-08-24, confirmado por DESCRIBE); el formulario público sigue sin generar clave de acceso al sistema (confirmado por diagnóstico que esa tarea nunca vivió ahí — es y sigue siendo exclusiva del coordinador vía matricular en est_mdl.php); calcularEstadoFicha() no se modificó; verificado en navegador 15/15 pruebas + Edición manual de matr_numero en el modal "Editar Matrícula" de 02_estudiantes (commit `59775e4`): campo antes de solo lectura (`#spn_editar_matr_numero`) se vuelve editable (`#npt_editar_matr_numero`, `type="number"`), con validación de unicidad server-side en el case `editar_matricula` de `est_mdl.php` (mismo patrón que `doce_sigla` en `doc_mdl.php` — SELECT previo excluyendo el propio registro, mensaje específico en vez de catch genérico); `pdf_hoja_matricula.php` mejora el manejo de errores de la asignación automática distinguiendo colisión de unicidad (SQLSTATE 23000) de errores genéricos; el mecanismo de asignación automática (SELECT ... FOR UPDATE sobre configuracion + MAX(matriculas.matr_numero) + GREATEST) no se modificó — confirmado por diagnóstico que relee MAX en vivo en cada asignación, sin contador cacheado que desincronizar con la edición manual; no se tocó el modal "Completar Matrícula" + Reestructuración del modal de creación de estudiante en 02_estudiantes (commit `f088466`), fuera del alcance original de las Fases A–F pero sobre el mismo modal unificado `#mdl_estudiante`: título de creación renombrado de "Nuevo Aspirante" a "Ficha de Inscripción" (edición conserva "Editar Estudiante"), nuevo bloque de encabezado "FICHA DE INSCRIPCIÓN — código AC-FO-02" con instrucciones y advertencia de pérdida de datos al cerrar sin guardar (reemplaza las instrucciones antiguas de "Ficha Familiar (AC-FO-02)"), 5 secciones del modal numeradas con nueva clase `.seccion-titulo` (00_files/estilos.css) — sección 3 renombrada de "Ficha Familiar (AC-FO-02)" a "Información sobre Familiares", sección 5 renombrada de "Información general" a "Programa técnico al que ingresa" y reubicada al final del modal (antes intercalada justo antes de Padre/Madre/Acudiente) —, nueva clase `.campo-lleno` (fondo verde) aplicada dinámicamente vía nueva función `marcarCampoLleno()` con listener delegado sobre los inputs/selects/textareas de `#mdl_estudiante`, capa visual independiente de `marcarValidacion()`/`is-invalid`/`is-valid` sin modificarla; se evaluó agregar `#bloque_foto_estudiante` también al modo creación y se descartó porque la subida de foto requiere un `estu_id` ya existente (el endpoint `subir_foto` no puede ejecutarse antes del primer guardado), la foto permanece exclusiva del modo edición sin cambios; verificado en navegador 8/8 pruebas + Limpieza de código muerto tras el plan de unificación de ficha de estudiante (commit 5de9a9e): elimina los 4 cases sin llamadores (guardar/obtener/obtener_ficha/guardar_ficha) de est_mdl.php y abrirFicha()/handler btn_guardar_ficha de est_ctrl.js, documentados como pendientes de limpieza desde los commits 42e4175/76f90c5; limpiarFormularioFicha() conservada por tener llamador activo real; cierra la deuda técnica de las Fases C1/C2 + Fase F2 del plan de unificación de ficha de estudiante, cierra el plan completo A–F: nuevo pdf_hoja_matricula.php (06_reportes, AC-FO-09) con dompdf Carta portrait, excluye Nivel y checklist de Requisitos, asignación segura de matr_numero bajo concurrencia (SELECT ... FOR UPDATE sobre configuracion, primer uso de este patrón en el proyecto), matriculas resuelto como 1:N por estudiante vía ORDER BY matr_id DESC LIMIT 1 (aplicado también en obtener_completo de est_mdl.php), validación server-side de matr_estado === 'matriculado', botón "🖨️ Hoja de Matrícula" en el modal unificado de 02_estudiantes visible solo si matriculado + Fase F1 del plan de unificación de ficha de estudiante: columna usua_nombre nueva en usuarios (prerequisito de la Fase F2, PDF Hoja de Matrícula) — descubierta como faltante durante el diagnóstico de Fase F (ningún Admin/Coordinador tenía nombre legible, solo usua_email), CRUD completo en 08_admin con validación server-side simétrica entre crear/editar, propagada a $_SESSION['usua_nombre'] en login; fila F del roadmap dividida en F1 (este commit) y F2 (PDF, pendiente), mismo criterio ya usado para dividir C en C1/C2 + Fase E del plan de unificación de ficha de estudiante: reactivación de matr_folio/matr_observacion (removidos deliberadamente de la UI en 29b6ca6, backend intacto desde entonces) en los modales Completar Matrícula y Editar Matrícula de 02_estudiantes, fechamatricula deja de fijarse con CURDATE() hardcodeado y se vuelve editable, matr_numero cambia de VARCHAR(20) a INT UNSIGNED con UNIQUE KEY nueva (múltiples NULL confirmados válidos en MySQL/InnoDB, conversión verificada sin pérdida de datos), matr_numero queda de solo lectura en ambos modales sin lógica de asignación automática (reservada para Fase F) + Fase D del plan de unificación de ficha de estudiante: configuración institucional en 08_admin — tabla `configuracion` nueva de fila única (config_id fijo en 1, CHECK constraint chk_configuracion_fila_unica), columnas tipadas matr_numero_inicial/director_nombre/secretario_nombre (primer caso del proyecto de un registro de configuración fijo, sin PK variable), 2 cases nuevos en admin_mdl.php con guard admin-only replicado del resto del módulo, bloque nuevo sin tabs en admin_view.php con texto de ayuda sobre aplicación hacia adelante, precarga/guardado en admin_ctrl.js, fix incidental de estilos.css faltante en admin_view.php + Fase C2 (frontend) del plan de unificación de ficha de estudiante, cierra la Fase C completa: modal mdl_estudiante ampliado a modal-xl y absorbe todo el contenido de mdl_ficha (eliminado por completo), abrirEditar() precarga estudiante + Ficha Familiar en un solo flujo vía obtener_completo (incluye estado_ficha, visibilidad Padre/Madre, cascada Acudiente), guardado unificado en btn_guardar_estudiante hacia guardar_completo con payload de objeto plano (no FormData, por convención de CLAUDE.md), nuevo botón "🖨️ Ficha Inscripción" condicional a estado_ficha === 'completa', botones de fila "📋 Ficha"/"🖨️ PDF" eliminados de tablaAspirantes/tablaMatriculados, cases/handlers antiguos (guardar/guardar_ficha/obtener/obtener_ficha/abrirFicha/btn_guardar_ficha) quedan como código muerto sin llamadores + Fase C1 (backend) del plan de unificación de ficha de estudiante: nuevo case guardar_completo en est_mdl.php (02_estudiantes) unifica creación/edición de estudiante + Ficha Familiar en una sola transacción PDO (antes 2 transacciones separadas en 2 requests, sin atomicidad real), Ficha Familiar ahora obligatoria siempre sin excepción por modo, nuevo case obtener_completo combina estudiante + ficha en un solo JSON vía LEFT JOIN + Fase B del plan de unificación de ficha de estudiante: columna "Edad" en tbl_matriculados de 02_estudiantes, formato "17 (23 sep)" (edad calculada en JS ajustada por mes/día + cumpleaños abreviado con array local de meses en español, sin intl ni librería de fechas), resaltado celeste #cfe2ff embebido en render() para menores de 18, fechanacimiento agregado al SELECT de listar_matriculados, manejo defensivo de NULL con "—", tablaAspirantes sin cambios + Fase A del plan de unificación de ficha de estudiante: botón "Editar" renombrado a "📝 Datos Estudiante" en tablaMatriculados y tablaAspirantes de 02_estudiantes, adopta el punto de color e indicador de completitud (estado_ficha) que antes solo tenía el botón "Ficha" (botón "Ficha" se mantiene intacto hasta la Fase C) + precarga de Programa y Jornada declarados (fichas_inscripcion) en el modal Completar Matrícula de 02_estudiantes (Programa preseleccionado pero editable, Jornada solo informativa, sin persistirse en matriculas) + eliminación de Períodos en 04_grupos (DELETE condicionado, sin alternativa "desactivar" por la semántica de único-activo de peri_activo, con salvaguarda extra que nunca permite eliminar el período activo) + edición de matrícula (Editar Programa/Cohorte/Período con retiro automático de módulos huérfanos) + fix de causa raíz en Asignación Estudiantes (listar_estudiantes_disponibles ahora valida matrícula real, no solo coho_id) + filtros Programa/Período/Grupo/Módulo + columnas Cohorte/Módulos (con modal de detalle) en Matriculados (02_estudiantes) + columna de correo en Matriculados + cascada Programa→Cohorte→Período (cohorte ahora obligatoria) en modal Completar Matrícula (02_estudiantes) + CRUD de Programas en 04_grupos + reordenamiento de pestañas (Programas, Módulos, Períodos, Cohortes, Grupos Semestre, Asignación Estudiantes) + Ficha Familiar obligatoria + indicador de 3 estados (Fase 5 formulario público) + zona horaria America/Bogota en Docker + select de Reportes con nombre de módulo e inicial de docente + eliminar aspirante desde 02_estudiantes + CRUD completo de módulos en 04_grupos + fix de sesión faltante en listar_grupos (05_calificaciones) + jornada movida de cohorte a grupo semestre (grse_jornada) + fix de precarga/edición/eliminación en modal de módulo-grupo (04_grupos) + eliminar/desactivar cohorte con verificación de dependencias + fila de contexto del Excel exportado dividida en 2 filas + período activo (peri_activo) y conteo de grupos por docente + eliminar/desactivar docente y corrección de doce_activo desincronizado + texto de ayuda para siglas y corrección de maxlength/mayúsculas + filtros de profesor/programa/período en calificaciones + sidebar de ayuda con CRUD (10_ayudas) + sidebar de ayuda expandido a todos los módulos + fix refresco matriculados + cambio de clave para estudiantes matriculados + fix del select de sección en el modal de ayuda + guard de autorización por rol en listar_grupos (05_calificaciones) + período activo preseleccionado y sincronizado en modal Grupo Semestre + grse_codigo y peri_codigo autogenerados readonly permanente + modal Grupo Semestre ampliado a modal-xl + validación de unicidad de doce_sigla + eliminar_cohorte envuelto en transacción PDO + conteo de carga de docente en selector de asignación de módulo + desempate alfabético en listar_calificaciones + autenticación de estudiantes migrada a correo real (estu_email UNIQUE) + fix del selector de módulos del estudiante (DISTINCT + ONLY_FULL_GROUP_BY) + sincronización de contenido de 10_ayudas con el estado real del sistema tras diagnóstico (5 entradas de 02_estudiantes desactualizadas desde el 16-18 de agosto, corregidas contra los cambios de las Fases A-F del plan de unificación de ficha de estudiante; nueva entrada de Configuración Institucional en 08_admin; corrección de dos pares de ayud_orden duplicados en 04_grupos)
