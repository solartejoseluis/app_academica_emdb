...
> Versión: 102 — agrega la lista informativa de requisitos en el modal de matrícula (commit `b84eda2`), Etapa 2.12.H. `case 'listar_requisitos_programa'` (`est_mdl.php`) gana Coordinador al guard (`!in_array($role_id, [1, 2], true)`, antes `!== 1`) — único cambio en ese archivo, confirmado que no toca los otros 4 `case` del catálogo (siguen Admin-only, es la única consulta de solo lectura del grupo). Nuevo bloque en `#mdl_matricular` (`est_view.php`), en la misma posición donde vivían los 9 checkboxes fijos retirados en la Etapa 2.12.A2 (entre "Período" y "Datos de matrícula"): lista de solo lectura o mensaje "sin catálogo", ambos ocultos por defecto. El handler `change` de `#slct_prog_id` gana una llamada a la nueva `cargarRequisitosInfoMatricula(prog_id)` (local al closure de `ready()`, solo la usa ese handler) que filtra client-side `reqp_activo == 1` y puebla la lista solo con `reqp_nombre` — sin descripción ni checkbox; `abrirMatricular()` suma el reset de ambos bloques al reset ya existente, sin crear uno nuevo. Con esta etapa se completa el reemplazo funcional de los 9 checkboxes retirados en 2.12.A2 — el coordinador vuelve a ver, al matricular, qué se le va a pedir al estudiante, ahora derivado del catálogo configurable. Verificado con Playwright + `curl` en 5 casos: programa con requisitos (lista correcta), programa sin catálogo (mensaje esperado), reapertura del modal sin arrastrar datos, matrícula real completada sin error con `requisitos_estudiante` poblado correctamente (SQL directo), y confirmación por `curl` de que Coordinador ahora puede leer el catálogo pero sigue sin poder escribirlo. Datos de prueba eliminados al terminar. Decisión: se amplió el guard existente en vez de duplicar el endpoint — es una consulta de solo lectura y el proyecto ya distingue "leer" de "modificar" el catálogo; a diferencia de `cargarProgramas()` en la Etapa 2.12.E2, aquí no había ninguna razón técnica para no reutilizar. Restan 2 etapas del roadmap: 2.12.I (vista del Estudiante en `06_reportes`) y 2.12.J (documentación de cierre).
