...
> Versión: 99 — agrega el listado real de requisitos dentro del modal de matrícula (commit `9196f18`), tercera de 4 sub-entregas de la Etapa 2.12.F. El ítem "📋 Requisitos" del dropdown ahora pasa también el nombre del estudiante en el `onclick`, reutilizando el patrón de escape de apóstrofes ya existente en `est_ctrl.js` (`.replace(/'/g, "\\'")`, ya usado en `verModulosEstudiante()`/`abrirAvanzarSemestre()`). `abrirRequisitosMatricula(matr_id, nombreEstudiante)` fija el título dinámico y, tras abrir el modal, llama a la nueva `cargarRequisitosMatricula(matr_id)`: `AJAX GET` a `listar_requisitos_matricula`, que muestra el mensaje "sin catálogo" o puebla la tabla con una fila por requisito — numeración, nombre, descripción, un `<select>` `pendiente`/`entregado` preseleccionado según el estado real (`data-reqe-id`, sin handler de guardado todavía) y la fecha formateada dd/mm/aaaa o `—` (mismo patrón inline ya usado en `cargarTablaRequisitosPrograma()`) — y calcula el texto de progreso "`x` de `y` requisitos completados". Verificado con Playwright headless en 3 casos: matrícula con requisitos mixtos (1 entregado, 1 pendiente) mostrando título, filas y progreso correctos; matrícula de un programa sin catálogo mostrando el mensaje esperado con la tabla oculta; y un cambio manual de `<select>` en el DOM sin ningún error de consola ni persistencia (esperado en esta sub-entrega). Datos de prueba eliminados al terminar. Decisión: el nombre del estudiante se pasa desde el frontend (ya disponible en la fila) en vez de agregar un `JOIN` a `estudiantes` en `listar_requisitos_matricula` — mismo criterio ya aplicado en `abrirEditarRequisitoPrograma()`, mantiene el endpoint con una sola responsabilidad. Etapa 2.12.F3 (de 4) del roadmap "Gestión de requisitos de estudiantes".
