...
> Versión: 103 — agrega el backend de resumen de requisitos para la vista del Estudiante (commit `b745774`), primera de 2 sub-entregas de la Etapa 2.12.I. Nuevo `case 'resumen_requisitos_estudiante'` en `reportes_mdl.php` (`06_reportes`), guardado con el mismo patrón de `mis_programas` (`in_array($role_id, [1, 2, 4], true)`) y resolviendo el estudiante objetivo con `resolverEstuIdObjetivo()` (reutilizada, no reimplementada). Filtra explícitamente `matr_estado = 'matriculado'` — solo esas matrículas reciben filas en `requisitos_estudiante` por el backfill de la Etapa 2.12.C — y trae, por cada matrícula, 2 subqueries correlacionadas (`total`/`entregados`), mismo patrón ya usado en `listar_matriculados` (`02_estudiantes`), aplicado aquí por primera vez en `06_reportes`. Responde `total_global`/`entregados_global` (suma en PHP de todas las matrículas) y `por_matricula` (el detalle por matrícula que la Etapa 2.12.I2 usará para decidir qué pestañas muestran badge) — el detalle fila por fila de cada requisito sigue pidiéndose aparte con `listar_requisitos_matricula` (ya existente, Etapa 2.12.D), por pestaña, deliberadamente sin duplicar esa consulta aquí: el resumen alimenta la barra global, el detalle solo se pide cuando el estudiante abre la pestaña. Verificado con `curl` autenticando como un estudiante real de prueba (usuario propio con login): 1 matrícula con catálogo (totales coinciden con la única fila), 2 matrículas en programas distintos (`total_global` es la suma real, `2+1=3`), una 3ª matrícula `'retirado'` (respuesta idéntica al caso anterior, esa matrícula no aparece ni suma), y un intento como Docente (`role_id=3`) rechazado con "Sin autorización". Datos de prueba eliminados al terminar. Restan la Etapa 2.12.I2 (HTML/JS: barra global + sección de detalle por pestaña) y 2.12.J (documentación de cierre).
