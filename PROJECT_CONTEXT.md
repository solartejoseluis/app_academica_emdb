...
> Versión: 100 — agrega el wiring de guardado del checklist de requisitos por matrícula (commit `a5ef1e5`), cuarta y última sub-entrega de la Etapa 2.12.F — con esta versión se cierra por completo esa etapa (F1 a F4): el checklist de requisitos por matrícula es funcional de punta a punta para el Coordinador (ver columna resumen en el listado, abrir el detalle por matrícula, marcar/desmarcar cada requisito con persistencia y refresco inmediato, sin salir de `tablaMatriculados`). Nuevo handler `change` delegado sobre `.select-estado-requisito` (`$(document).off('change.requisitosMatricula', ...).on('change.requisitosMatricula', ...)`, dentro de `cargarRequisitosMatricula()`), mismo patrón de delegación ya usado en el archivo (`marcarCampoLleno`) — el `.off()` con namespace es obligatorio porque esa función corre en cada apertura del modal, y sin él cada apertura apilaría un handler adicional. En éxito, actualiza al instante la fecha de la fila (`new Date()` en JS, sin pedírsela al backend) y recalcula el progreso contando los `<select>` en `'entregado'` sobre el total, ambos sin volver a pedir el listado completo; refresca además la columna "Requisitos" de la tabla principal vía `$('#tbl_X').DataTable().ajax.reload(null, false)`, recuperando la instancia por selector DOM — no por variable directa, porque `tablaMatriculadosActual`/`tablaMatriculadosAnteriores` son `let` locales al closure de `$(document).ready()`, inaccesibles desde este handler global — recargando solo la pestaña activa. En error, revierte el `<select>` a su valor previo (`data-valor-previo`) y muestra `alert()`. Sin `confirm()` — a diferencia de `desactivarRequisitoPrograma()`, cambiar `pendiente`/`entregado` no es destructivo, se revierte con otro click. Verificado con Playwright headless en 4 casos: marcar entregado (fecha y progreso correctos, confirmado con SQL directo), revertir a pendiente (`reqe_fecha = NULL` confirmado), refresco del badge en la tabla principal tras cerrar el modal sin recargar la página, y un error de red simulado (interceptando la ruta, sin tocar el backend real) revirtiendo el select correctamente. Datos de prueba eliminados al terminar. Con la Etapa 2.12.F cerrada, restan 4 etapas del roadmap total: 2.12.G (ajustes de columnas), 2.12.H (checklist informativo en `#mdl_matricular`), 2.12.I (vista del Estudiante en `06_reportes`) y 2.12.J (documentación de cierre).
