...
> Versión: 94 — asigna requisitos automáticamente al crear una matrícula nueva (commit `f14331d`), cerrando el lado de escritura del backend iniciado en la v93: en `case 'matricular'` (`est_mdl.php`), rama de creación nueva (`INSERT`, no la rama `$existingMatr` que hace `UPDATE`), inmediatamente después de `$matr_id_nuevo = $pdo->lastInsertId()` se inserta un registro `'pendiente'` en `requisitos_estudiante` por cada requisito activo del programa (`INSERT ... SELECT ... FROM requisitos_programa WHERE prog_id = ? AND reqp_activo = 1`), dentro de la misma transacción PDO que ya envolvía el `INSERT`/`UPDATE` de `matriculas` y la creación de acceso opcional — sin abrir ninguna transacción nueva. La rama `$existingMatr` (re-matrícula sobre la misma fila) no se tocó — confirmado con Jose Luis que no lo necesita, porque el `matr_id` no cambia y los requisitos ya existen desde la creación original. Verificado con `curl` contra Docker vivo, autenticando como Coordinador (`role_id = 2`, el rol real que matricula en producción): matriculando en un programa con 2 requisitos activos de prueba, la matrícula nueva recibió exactamente 2 filas `'pendiente'` con el `matr_id` correcto; matriculando en un programa sin ningún requisito activo, la matrícula se creó sin error y con 0 filas (comportamiento esperado del `INSERT ... SELECT` sobre un `SELECT` vacío, sin manejo especial); re-matriculando sobre la misma fila estu+prog+peri, el conteo de `requisitos_estudiante` se mantuvo igual antes y después mientras `matr_observacion` sí se actualizó, confirmando que se tomó la rama `UPDATE`. Datos de prueba (estudiante temporal, 2 matrículas, 2 requisitos de catálogo) eliminados al terminar. Etapa 2.12.C (de 10) del roadmap "Gestión de requisitos de estudiantes"; `est_view.php`/`est_ctrl.js` siguen sin tocarse (Etapa 2.12.E).
