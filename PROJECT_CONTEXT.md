...
> Versión: 101 — ajusta el ancho de las columnas Edad/Programa/Semestre en `tablaMatriculados` (commit `4386b67`), Etapa 2.12.G, sin dependencias con el resto del roadmap. 2 `<th>` abreviados en ambas tablas (`est_view.php`): "Programa" → "Prog", "Semestre" → "Sem" ("Edad" sin cambio de texto). 3 anchos ajustados en `crearColumnasMatriculados()` (`est_ctrl.js`), sin tocar ningún `render()`: Edad `100px → 85px` (contenido real hasta ~12 caracteres, `"N (dd mes)"`, más el padding del badge azul de menor de edad); Programa `70px → 85px` — **ampliado, no reducido**, porque el peor caso real del sistema no son las siglas cortas (`ASO`/`MD`) sino `ASO2016`/`AMD2016` (7 caracteres, programas ya existentes en el catálogo); Semestre `60px → 50px` (contenido real máximo `"N/M"`, 3 caracteres, con margen para el badge verde del último semestre). Cada columna se dimensionó por el peor caso real confirmado, no por el típico — mismo criterio ya documentado en CLAUDE.md para "código autogenerado por concatenación" — lo que llevó a que Programa terminara más ancho que lo sugerido originalmente (50px), porque ese pedido inicial no había considerado las variantes largas ya presentes en datos reales. Verificado con Playwright headless en ambas tablas: encabezados correctos, sin overflow horizontal en ningún caso (medido con `scrollWidth > clientWidth`), incluyendo el peor escenario real probado con un estudiante y matrícula temporales en `ASO2016` (semestre 3/3, último, con edad de 2 dígitos y badge azul) — comparado también contra siglas cortas y semestres típicos ya existentes; resto de columnas sin desalinear. Datos de prueba eliminados al terminar. Restan 3 etapas del roadmap: 2.12.H (checklist informativo en `#mdl_matricular`), 2.12.I (vista del Estudiante en `06_reportes`) y 2.12.J (documentación de cierre).
