...
> Versión: 90 — resuelve 4 pendientes menores de "Deuda técnica" en un solo commit (`6abbdae`): (1) centraliza `configuracion.institucion_nombre` en los 3 PDFs que seguían con el nombre institucional hardcodeado — `pdf_grupo.php`, `pdf_hoja_matricula.php`, `pdf_ficha.php` — mismo patrón que `pdf_boletin.php` (`ef429bd`); (2) `aprobar_actualizacion` (`est_mdl.php`) cambia su `SELECT * FROM estudiantes` por columnas explícitas derivadas de `$mapaEstudiantes`, prerequisito del punto 3; (3) elimina `estudiantes.coho_id` (columna + FK `fk_estu_coho`), sin escrituras desde `3e551e5` y, tras el punto 2, sin ningún lector activo — aplicado contra Docker vivo y reflejado en `database/emdb_academica.sql`, con el comentario de `matriculas` corregido; (4) elimina los `case` `mis_modulos`/`mis_notas` de `reportes_mdl.php`, código muerto confirmado sin llamador en todo el proyecto. Probado contra Docker vivo: los 4 PDFs verificados con `pdftotext` (nombre institucional idéntico en los 4); `aprobar_actualizacion` probado con una solicitud simulada (aplicó campos, sincronizó email, datos revertidos); `listar_matriculados`/`obtener_completo` probados tras el `DROP COLUMN` sin errores; `php -l` limpio en los 6 archivos modificados.
