...
> Versión: 96 — agrega la ficha "Configurar Requisitos" para Admin (commit `f3c0ca4`), la primera pieza de FRONTEND de la Fase 2.12 (las versiones v92-v95 fueron exclusivamente esquema y backend). Nueva pestaña en `est_view.php`, visible solo si `role_id === 1`, replicando la forma del condicional `in_array([1, 2])` que ya usa el archivo pero reducida a un único rol: select de programa, tabla de requisitos con filas atenuadas (`table-secondary text-muted`) para los inactivos, y modal `#mdl_requisito_programa` (nombre + descripción) reutilizado tanto para Agregar como para Editar. `listar_programas` (`est_mdl.php`) gana `prog_activo` como columna aditiva — sin `WHERE` nuevo, para no cambiar el comportamiento de los demás consumidores del endpoint (selects de matrícula) — y el filtro a "solo activos" se aplica únicamente client-side, en una función nueva (`cargarProgramasConfigRequisitos()`) con su propia llamada AJAX en vez de reestructurar la `cargarProgramas()` ya estable. El resto del JS (`est_ctrl.js`) sigue el patrón obligatorio del proyecto: `abrirEditarRequisitoPrograma()`/`desactivarRequisitoPrograma()`/`reactivarRequisitoPrograma()` en scope global (invocadas vía `onclick` inline), sin SweetAlert (confirmado que el proyecto no lo usa en ningún archivo) — todo con `alert()`/`confirm()` nativos. Verificado en 3 niveles: `curl` para crear/editar el catálogo; Playwright headless, instalado puntualmente en esta sesión sin quedar como dependencia del proyecto, para confirmar visualmente que el select solo lista programas activos (desactivando y reactivando un programa real de forma reversible); y en navegador por Jose Luis, el ciclo completo agregar → editar → desactivar → reactivar → matricular un estudiante con esos requisitos ya asignados. `eliminar_requisito_programa` se mantiene como borrado lógico exclusivo — sin botón de borrado físico en esta UI, misma decisión de la Etapa 2.12.B por el FK `RESTRICT`. Etapa 2.12.E (de 10) del roadmap "Gestión de requisitos de estudiantes".
