...
> Versión: 97 — agrega la columna "Requisitos" de solo lectura en `tablaMatriculados` (commit `8450a24`), primera de 4 sub-entregas de la Etapa 2.12.F. Nuevo `<th>Requisitos</th>` en ambas tablas (`tbl_matriculados_actual`/`tbl_matriculados_anteriores`, `est_view.php`), entre "Información" y "Cohorte", y su columna correspondiente en `crearColumnasMatriculados()` (`est_ctrl.js`): `'—'` si `requisitos_total` es `0`; si no, badge `x/y` — verde si `entregados === total`, amarillo en cualquier otro caso (sin distinguir "0 entregados" de "parcial", mismo color para ambos por simplicidad) — con la fecha de última actualización debajo cuando existe, mismo patrón inline de formateo ya usado en `cargarTablaRequisitosPrograma()`. Columna puramente informativa en esta sub-entrega, sin ninguna acción — el modal de edición llega en 2.12.F2/F3/F4. **Bug encontrado y corregido durante el desarrollo:** el cambio se implementó primero solo en `est_ctrl.js` (16 columnas en el array `columns`) sin su `<th>` correspondiente en `est_view.php` (que seguía en 15) — el alcance original del cambio excluía explícitamente ese archivo. Esto rompía DataTables por completo en ambas tablas (`pageerror: Cannot read properties of undefined (reading 'style')`, `tbody` vacío, cero filas). Se diagnosticó con `git stash` — revirtiendo temporalmente el cambio de JS y repitiendo la prueba en navegador — que la causa era exclusivamente el desajuste 15 vs 16 columnas, no los errores de CORS del CDN `i18n` de DataTables ya conocidos en este entorno (inofensivos, confirmado que la tabla original renderizaba bien pese a ellos). Corregido en un cambio separado agregando el `<th>` faltante en las 2 tablas. Verificado con Playwright headless: sin `pageerror`, encabezados=celdas en ambas pestañas, los 3 escenarios de badge (`'—'`, amarillo `0/1`, verde `1/1` con fecha) confirmados con datos de prueba reales, eliminados al terminar. Etapa 2.12.F1 (de 4) del roadmap "Gestión de requisitos de estudiantes".
