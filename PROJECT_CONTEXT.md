...
> Versión: 98 — agrega el dropdown + modal (solo estructura) para requisitos por matrícula (commit `caa3cff`), segunda de 4 sub-entregas de la Etapa 2.12.F. Nuevo modal `#mdl_requisitos_matricula` (`est_view.php`, `modal-lg`, mismo estilo Bootstrap 5 que los demás modales del archivo): título estático (se completará con el nombre del estudiante en la Etapa 2.12.F3), hidden `#npt_matr_id_requisitos`, bloque de mensaje para programas sin catálogo, tabla `#tbl_requisitos_matricula` (#, Requisito, Descripción, Estado, Fecha) y `#txt_progreso_requisitos_matricula` en el footer — estructura completa, sin datos todavía. Nuevo ítem "📋 Requisitos" en el dropdown de Acciones (`crearColumnasMatriculados()`, `est_ctrl.js`), insertado entre "📝 Datos Estudiante" y "🔗 Generar link actualizar datos" — posición confirmada por un diagnóstico previo del orden real del dropdown antes de decidir dónde insertarlo. A diferencia de los demás ítems condicionales de esa columna (que solo aparecen en Per. Actual), "Requisitos" aparece siempre en ambas pestañas — decisión explícita: un coordinador puede necesitar revisar o completar requisitos de una matrícula de un período ya finalizado, no solo del actual. Nueva función global `abrirRequisitosMatricula(matr_id)` que en esta sub-entrega solo fija el hidden y abre el modal, sin AJAX todavía. Verificado con Playwright headless en ambas tablas: el ítem aparece en el 100% de las filas de ambos datasets probados, el modal abre con el `matr_id` correcto y cierra bien tanto con "Cerrar" como con la X, sin ningún `pageerror`. Etapa 2.12.F2 (de 4) del roadmap "Gestión de requisitos de estudiantes".
