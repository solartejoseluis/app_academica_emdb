...
> Última actualización: 2026-09-20 — último commit citado: 7a52825
...
app_academica_emdb/
  ...
  scripts/
    export_para_hosting.sh — Genera un backup de la BD listo para subir al hosting (ver "Checklist de deploy a producción")
  database/
    emdb_academica.sql — Seed versionado en git: estructura + catálogos fijos, nunca datos reales de estudiantes (repo público)
    hosting_deploy/    — Snapshots con datos reales generados por scripts/export_para_hosting.sh, fuera de git salvo .gitkeep
  CLAUDE.md
  README.md
  CHANGELOG.md
```
...
### Modal con formulario: `data-bs-backdrop="static" data-bs-keyboard="false"` para no perder lo escrito con un clic afuera o Esc

Por defecto, Bootstrap 5.3 cierra cualquier `.modal` con un clic fuera de él o con la tecla Esc, descartando en silencio cualquier dato ya diligenciado en el formulario que contiene. Todo modal **nuevo** que contenga un formulario (campos que el usuario diligencia y que se perderían al cerrarse por accidente) debe llevar ambos atributos en su `<div class="modal ...">`:

```html
<div class="modal fade" id="mdl_ejemplo" tabindex="-1" aria-hidden="true"
     data-bs-backdrop="static" data-bs-keyboard="false">
```

Con esto, el modal solo se cierra por una acción explícita: el botón X del header o un botón Cancelar/Cerrar, ambos con `data-bs-dismiss="modal"` (o su equivalente por JS) — verificar que el modal tenga al menos uno de los dos antes de agregar estos atributos, porque sin ningún medio de cierre explícito el modal quedaría atrapado.

**Modales de solo lectura y de confirmación quedan sin estos atributos** — conservan el comportamiento por defecto (clic afuera/Esc cierran) porque no hay ningún dato diligenciado que perder: los 8 `mdl_confirmar_eliminar_*` del proyecto, y los modales de listado de solo lectura (`mdl_estudiantes_grupo`, `mdl_modulos_estudiante`).
...
### Seed versionado en git (`database/emdb_academica.sql`) y snapshot con datos reales (`database/hosting_deploy/`) son archivos deliberadamente separados, nunca sincronizados entre sí

- **Contexto:** el repositorio GitHub de este proyecto es **público**. `scripts/export_para_hosting.sh` (commit `40b28c8`, 2026-09-06) automatiza la generación de un backup con datos reales de estudiantes para subir al hosting (pruebas o producción, vía cPanel/phpMyAdmin).
- **Decisión:** `database/emdb_academica.sql` (versionado en git) contiene únicamente estructura y datos base/de ejemplo — **nunca** datos reales de estudiantes (nombres, documentos, notas, contactos). Los snapshots con datos reales generados por el script viven exclusivamente en `database/hosting_deploy/`, agregada a `.gitignore` (excepto `.gitkeep`) — nunca se sube a GitHub, uso puramente local para la subida manual al hosting.
- **Razón:** exponer datos personales de estudiantes en un repositorio público violaría la Ley 1581 de 2012 (Habeas Data) — la separación no es una preferencia de organización de carpetas, es el mecanismo que evita esa exposición.
- **Consecuencia:** `scripts/export_para_hosting.sh` nunca escribe ni modifica `database/emdb_academica.sql`. No crear ningún mecanismo que sincronice o actualice el seed público a partir de un dump con datos reales sin decisión explícita de Jose Luis.
- **Aplicada en la práctica (commit `300cfc1`, 2026-09-06):** tras el despliegue exitoso de la aplicación en producción (`app.escuelamdb.com`) con los datos reales de 54 estudiantes/aspirantes y 5 docentes que hasta entonces también vivían en `database/emdb_academica.sql`, el contenido de ese archivo se reemplazó por completo con un dataset anonimizado (nombres, cédulas y datos de ficha familiar generados aleatoriamente para los mismos 54 estudiantes/aspirantes y 5 docentes). Los usuarios Administrador y Coordinador conservaron su `usua_passwordhash` real — mismo acceso de siempre — excepto un usuario Coordinador que tenía el nombre real de una persona de la institución y fue anonimizado también (nombre y correo, hash sin tocar). Detalle completo en CHANGELOG.md.
- **Estado:** Activa. No negociable (mismo nivel que las demás decisiones de esta sección) — desde `300cfc1`, el seed público ya no contiene ningún dato real; cualquier carga inicial futura debe mantener este mismo criterio.

### El docente opera solo sobre el período activo

- **Contexto:** desde la migración histórica de 2026-1 (2026-09-05), la BD tiene módulos con notas de períodos ya cerrados. Antes del commit `cef939b` (2026-09-19), ningún endpoint del docente en `05_calificaciones`/`06_reportes` (vista del docente) filtraba por período — solo verificaba pertenencia del `grmo_id` al docente en sesión, permitiendo ver y editar notas de períodos cerrados.
- **Decisión:** todo endpoint que el docente use para listar, leer o escribir calificaciones exige además `periodos.peri_activo = 1` para el `grmo_id` en cuestión — no solo ownership por `docentes.usua_id`. Aplicado en 11 ubicaciones: `listar_grupos` (rama `role_id=3`) y las demás consultas de ownership de `calificaciones_mdl.php`, más `reporte_grupo` (`reportes_mdl.php`) y `pdf_grupo.php`. Mismo mensaje genérico de rechazo ante un `grmo_id` ajeno o de un período cerrado — no se distingue una causa de la otra.
- **Razón:** coordinador y administrador necesitan seguir viendo y operando sobre todos los períodos (históricos incluidos) para reportes, correcciones y consulta; el docente no — su rol operativo es cargar notas del período en curso, nunca reabrir un período cerrado.
...
### Recálculo del N3 de todo el roster al cambiar el conjunto de actividades

- **Contexto:** `recalcularN3()` promedia solo si el estudiante tiene nota en todas las actividades del `grmo_id`, y guarda el resultado en `calificaciones.cali_n3`. Crear o eliminar una actividad cambia esa regla para todo el grupo, pero el recálculo solo ocurría al guardar una nota (un estudiante a la vez), dejando `cali_n3`/`cali_nota_final`/`cali_definitiva` desfasados en planilla, reportes, boletín y `avanzar_semestre`.
- **Decisión:** helper `recalcularGrupoModuloN3($pdo, $grmo_id)` en `calificaciones_mdl.php`, invocado por `guardar_actividad_n3` (solo al crear) y `eliminar_actividad_n3` dentro de la misma transacción que el `INSERT`/`DELETE`. Recorre el roster vigente, reutiliza `recalcularN3()`/`recalcularNotaFinalYDefinitiva()`, solo actualiza filas de `calificaciones` existentes (nunca crea) y solo si `cali_n3` cambia. Sin triggers ni procedimientos almacenados.
- **Guarda:** al crear, el helper solo corre si el grupo ya tenía >= 1 actividad. Los grupos históricos (N3 cargado por la migración, 0 actividades) no se tocan: `recalcularN3()` daría `NULL` y borraría N3, nota final y definitiva.
- **Alternativas descartadas:** recálculo perezoso al abrir la planilla (convertía un endpoint de lectura en escritura y los reportes seguían con valores viejos); solo scripts SQL manuales (el problema reaparecía con cada actividad creada).
- **Limitación conocida:** la guarda solo protege al helper. En un grupo histórico donde se crea la primera actividad, el N3 histórico convive con una actividad sin notas hasta que se guarda una nota (ahí `guardar_nota_n3` recalcula solo a ese estudiante, comportamiento sin cambios).
- **Regla operativa:** cualquier endpoint futuro que cambie el conjunto de actividades de un `grmo_id` debe llamar al helper en la misma transacción y respetar la guarda.
- **Estado:** Activa desde el commit `7a52825` (2026-09-20).
...
- **Registro de errores de PHP en todos los entornos:** `display_errors` Off, `log_errors` On y `error_log` con ruta absoluta fuera de la raíz web (por ejemplo `/home/<cuenta>/logs/php.error.log` en producción y `php.error.staging.log` en staging), un archivo por entorno. No desactivar `log_errors`: sin registro no hay diagnóstico. No dejar `error_log` con ruta relativa: PHP crea un archivo `error_log` dentro de cada carpeta donde falla un script, sin extensión y fuera del alcance de `FilesMatch` por extensiones; si aparece uno, la raíz debe tener `<Files "error_log">` con `Require all denied`. Los cambios se hacen solo desde MultiPHP INI Editor (dominio por dominio) y luego se comprueba el `.htaccess` de la raíz. Evidencia: 2026-09-20, ver CHANGELOG.md.
...
| Vaciado de tablas en el hosting de pruebas — `information_schema` no accesible | El usuario de BD del hosting de pruebas (`aurusmind.com`, confirmado 2026-09-06) no tiene permiso `SELECT` sobre `information_schema` (error #1044), lo que impide vaciar todas las tablas vía una consulta SQL dinámica (el patrón que normalmente se usaría para generar los `DROP`/`TRUNCATE` de todas las tablas a la vez). Procedimiento manual mientras esto no cambie: en phpMyAdmin, ejecutar `SET FOREIGN_KEY_CHECKS=0;`, luego en la pestaña Estructura seleccionar todas las tablas y usar la acción "Eliminar" (Drop) — nunca una consulta SQL dinámica contra `information_schema` en ese hosting. Relacionado con `scripts/export_para_hosting.sh` (commit `40b28c8`, 2026-09-06), que resuelve el problema de colación de ese mismo hosting pero no este. **Nota (a 2026-09-20):** el staging usado es `dev.escuelamdb.com`, misma cuenta que producción; el texto de `aurusmind.com` de esta fila se conserva como registro de lo confirmado el 2026-09-06. |
...
| ~~`eliminar_actividad_n3` no recalcula `cali_n3`/`cali_nota_final`/`cali_definitiva` tras borrar una actividad~~ (RESUELTO 2026-09-20, commit `7a52825`) | **Resuelto:** el hueco era más grave al CREAR una actividad (estudiantes con N3 completo conservaban un promedio ya inválido) que al eliminar; ambos casos se resolvieron con el helper `recalcularGrupoModuloN3()` (ver "Recálculo del N3 de todo el roster..." en Decisiones arquitectónicas activas). El único valor desfasado detectado (`grmo_id` 30, estudiante 79) se corrigió con script en local, staging y producción (ver CHANGELOG, "Corrección de datos — N3 obsoleto"). Texto original del hallazgo, conservado: Descubierto durante el análisis previo al commit `8613a13` (2026-09-19, `05_calificaciones`): a diferencia de `guardar_nota_n3` (que sí llama a `recalcularN3()`/`recalcularNotaFinalYDefinitiva()` después de cada autosave), `eliminar_actividad_n3` solo ejecuta el `DELETE` — no recalcula nada para ningún estudiante afectado. Como `recalcularN3()` promedia sobre "todas las actividades del `grmo_id`" (sin columna de activo, ver más arriba), eliminar una actividad cambia ese total: un estudiante que antes tenía `cali_n3 = NULL` (por faltarle la nota de la actividad recién eliminada) podría ahora tener todas las notas completas para el nuevo total — pero `calificaciones.cali_n3` queda con el valor viejo (`NULL` u otro) hasta que el próximo autosave de una nota de ese estudiante dispare el recálculo. No es un problema nuevo introducido por `8613a13` — ya existía en `eliminar_actividad_n3` desde la Fase 2.14.B — pero se hizo más relevante al facilitar (con el roster vigente) que más actividades sean elegibles para eliminarse. Pendiente de decisión: si `eliminar_actividad_n3` debe recalcular `cali_n3`/Nota Final/Definitiva de todos los estudiantes del roster tras el `DELETE`, mismo patrón que `guardar_nota_n3`. No implementar sin decisión explícita de Jose Luis. |
| `guardar_nota` (`calificaciones_mdl.php`) tiene `cali_n3` en su whitelist | El endpoint acepta escribir `cali_n3` aunque la UI lo bloquee (`readonly`), y este archivo establece que `cali_n3` no se escribe directamente (ver "N3 configurable" en Reglas de negocio de calificaciones). Hallado al trabajar `7a52825` (2026-09-20). Riesgo bajo: requiere sesión y una petición manipulada. Pendiente de análisis porque quitarlo podría romper un flujo que hoy dependa de eso. No implementar sin decisión explícita de Jose Luis. |
| Migrar los N3 históricos como una actividad única por grupo ("N3 (migración histórica)") para eliminar la guarda de `7a52825` | **Pros:** sin caso especial, el modal mostraría las notas, recálculo seguro en todo grupo. **Contras:** cambio masivo de datos reales con script a mano en producción; crear una 2ª actividad en un grupo migrado dejaría el N3 en `NULL` (no existe "cierre de período" que lo impida); notas sintéticas que marcar; hay que revisar el formato de los N3 históricos y qué hacer con estudiantes retirados. No hacerlo durante la validación TRL5 (19 sep - 2 oct 2026). No implementar sin decisión explícita de Jose Luis. |
| Los tooltips (`title`) de los candados de Eliminar en "Configurar actividades" no se muestran en pantallas táctiles | Los indicadores "🔒 Con notas" y "🔒 Única actividad" (commit `8613a13`) explican el motivo solo en un `title` nativo, que no aparece al tocar; en celular el docente vería el candado sin explicación. Pendiente de saber si los docentes usan la app desde el celular. No implementar sin decisión explícita de Jose Luis. |

---

## Deuda técnica / pendiente — corrección de matrículas 2026-2 y período activo del docente (2026-09-19)

| Ítem | Detalle |
|---|---|
| Grupo B — 3 matrículas `'cursado'` de 2026-1 sin sucesora, con notas incompletas o promedio bajo | De las 15 matrículas `'cursado'` de la migración histórica sin fila sucesora en 2026-2 (ver CHANGELOG.md, corrección de datos del 2026-09-19), 3 (repartidas entre ASO/MD y AMD2016) no se repararon junto con las otras 6 porque tienen notas incompletas o un promedio que no alcanza para aprobar — no es un caso mecánico de "aprobó, crear sucesora". Pendiente de averiguar con los docentes correspondientes qué notas faltan o si el resultado del período es el correcto. **Aviso:** desde `cef939b`, el docente ya no puede cargar notas de 2026-1 (período cerrado) — si faltan notas por completar en estos 3 casos, debe cargarlas la coordinadora o el administrador, no el docente. |
| Grupo C — 6 estudiantes con indicios de ser datos de prueba de la migración histórica | Documentos ficticios o `NULL`, fechas de creación entre 2026-09-16 y 2026-09-18, todos etiquetados `matr_matriculadopor = 'Migración histórica 2026-1'`. Pendiente de decidir con Jose Luis si se eliminan o se corrigen con datos reales — no tocar sin esa decisión. |
| `avanzar_semestre` no crea filas en `requisitos_estudiante` para la matrícula sucesora | 21 de las 32 matrículas sucesoras existentes hoy no tienen ningún requisito asociado (a diferencia de `'matricular'`, que sí los asigna automáticamente al crear una matrícula nueva — ver Fase 2.12.C). Pendiente de decisión: si los requisitos de matrícula deben aplicarse una vez por estudiante o repetirse por cada semestre/matrícula sucesora. No implementar sin decisión explícita de Jose Luis. |
| Hueco de diseño — no existe una vista de "estudiantes sin matrícula en el período activo" | El ítem de dropdown "⏩ Matricular al sgte. sem." (Fase 2.10.E) solo aparece en la pestaña Per. Actual, sobre una matrícula ya existente de ese período — no hay ningún listado ni acción pensada para un estudiante que quedó sin ninguna matrícula en el período activo (ej. no se le generó la sucesora). Pendiente de decisión, relacionado con el Grupo B/C de esta misma sección. |
| Hallazgo — "Matricular en otro programa" puede crear una matrícula nueva de un programa ya cursado (no una matrícula duplicada — eso sí está bloqueado) | `case 'matricular'` (`est_mdl.php`, ~líneas 574-582) sí bloquea correctamente el caso de duplicado: `SELECT ... WHERE estu_id=? AND prog_id=? AND matr_estado='matriculado' AND peri_id != ?` rechaza crear una segunda matrícula del mismo programa mientras exista una fila `'matriculado'` vigente. El caso sin cubrir es distinto: si la única matrícula del estudiante para ese `prog_id` está en `'cursado'` (no `'matriculado'` — ej. una de las matrículas sin sucesora de la migración histórica, ver deuda técnica del 2026-09-19 más arriba), el chequeo no encuentra ninguna fila y `case 'matricular'` la acepta, creando una matrícula nueva desde cero para un programa que el estudiante ya cursó. El ítem "➕ Matricular en otro programa" se habilita mientras `programas_matriculados_activos < programas_activos_totales`, y `#mdl_matricular` no excluye programas ya cursados; a diferencia de `avanzar_semestre`, el semestre y la cohorte se eligen a mano (sin heredarse ni calcularse) y los `requisitos_estudiante` se asignan como en una matrícula nueva. Consecuencia: hoy es la única vía de la UI para matricular en el mismo programa, en un período nuevo, a un estudiante sin matrícula vigente de ese programa — un uso no previsto por el nombre del ítem. Pendiente de decisión (relacionado con el hueco de diseño "estudiantes sin matrícula en el período activo", fila de arriba). No implementar sin decisión explícita de Jose Luis. |
