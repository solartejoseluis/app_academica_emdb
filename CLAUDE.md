...
| `total_modulos` en el case `listar_grupos` (`grupos_mdl.php`, `04_grupos`) no filtra por `grmo_activo = 1`, a diferencia del `total_estudiantes` agregado al mismo case | Detectado al implementar `total_estudiantes` (commit `a39cb24`, 2026-08-30): la nueva subconsulta sí filtra `gm.grmo_activo = 1` — mismo patrón ya usado en `est_mdl.php` para contar módulos de un estudiante — pero `total_modulos`, ya existente en ese mismo `case 'listar_grupos'` desde antes de este commit, cuenta todos los `gruposmodulos` del grupo semestre sin ese filtro, incluyendo módulos inactivos. Es una inconsistencia preexistente entre dos columnas de la misma fila del listado, no introducida por este commit pero tampoco corregida por estar fuera de su alcance. Pendiente de decisión explícita de Jose Luis: ¿agregar `grmo_activo = 1` también a `total_modulos` para que ambos conteos sean consistentes, o dejarlo así porque contar módulos inactivos ahí es intencional? No implementar sin esa decisión. |
